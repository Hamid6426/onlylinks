  ...
  return (
    <div className="border border-gray-200 shadow-md p-6 rounded-md w-full mb-6">
      ...
      <p className="text-gray-600 mb-3">
        Display a sensitive content warning before visitors can view your
        profile.
      </p>
      ...
      <div className="flex items-center gap-3">
        <label className="relative inline-flex items-center cursor-pointer">
          ...
          />
          <div className="w-11 h-6 bg-gray-300 peer-focus:ring-4 peer-focus:ring-gray-300 rounded-full peer peer-checked:bg-gray-600"></div>
        </label>
            ...
            {["18", "21", "25", "0"].map((value) => (
              <label key={value} className="flex items-center gap-2">
                ...
                <span className="text-gray-700">
                  {value === "0"
                    ? "General Warning (No Age Restriction)"
                    : `${value}+`}
                </span>
              </label>
            ))}
      ...
      {/* Save Button */}
      <button className="mt-4 bg-gray-600 text-white px-4 py-2 rounded-md w-full">
        Save Changes
      </button>
    </div>
  );
